...
def _extract_dialogue(text: str) -> List[Dict[str, str]]:
    ...
    standard_pattern = re.compile(
        r"^[ \t]*(?!(?:INT\.|EXT\.|FADE |CUT TO|BLACKOUT))([A-Z][A-Z0-9\s\.\-']{1,30}(?:\s*\([^)]+\))?)[ \t]*\n"  # Character line
        r"((?:[ \t]*(?:\([^)]+\)[ \t]*\n)?(?:(?![ \t]*[A-Z]{2,}).+\n?)+))", # Dialogue block
        re.MULTILINE
    )
    ...
    if matches:
        ...
        for match in matches:
            ...
            # Additional check: character shouldn't be too long or just technical
            if len(character) > 40 or "TIMESTAMP" in character:
                continue
                
            # Clean up character name (remove parentheticals like (V.O.) for cleaner UI)
