...
class AuditoriumSimulation {
    ...
    initNetwork() {
        ...
        // Attach static listeners to our new buttons
        document.getElementById('btn-prev')?.addEventListener('click', () => window.sendCommand('PREV'));
        document.getElementById('btn-hold')?.addEventListener('click', () => window.sendCommand('HOLD'));
        document.getElementById('btn-go')?.addEventListener('click', () => window.sendCommand('NEXT'));

        this.socket.onopen = () => {
            console.log("✅ CONNECTED to Console");
            // Auto start the simulation since there is no start menu anymore
            window.sendCommand('START_SIM', 'fade_out');
        };
    ...
    renderConsole(state) {
        ...
        if (state.context_window) {
            ...
            state.context_window.forEach(cue => {
                ...
                // Parse scene_id from the display text format "scene_XXX â”‚ EMOTION â”‚ text"
                const parts = (cue.text || '').split('â”‚');
                ...
                if (isActive) {
                    ...
                    html += `<div class="cue-item active" ${clickAction}>
                                <div class="cue-header-row">
                                    ${sceneIdBadge}
                                    <span class="cue-separator">â”‚</span>
                    ...
                    // Inactive cue: compact â€” scene_id | short preview | dropdown
        ...
        if (btnHold) {
            if (state.is_holding) {
                ...
                btnHold.innerText = "RESUME â–¶";
            } else {
                btnHold.classList.remove('active');
                btnHold.innerText = "â¸ HOLD";
            }
        }
        ...
        const text = document.getElementById('time-display');
        const bg = document.getElementById('progress-bar');
        if (fill && text && typeof state.total_cues !== 'undefined') {
            ...
            if (handle) {
                handle.style.left = fill.style.width;
            }
        ...
        // â”€â”€ TELEPROMPTER UPDATE â”€â”€
        this.updateTeleprompter(state);
    }

    updateTeleprompter(state) {
        const tpText = document.getElementById('teleprompter');
        if (!tpText) return;
        ...
        if (!activeCue) {
            tpText.innerHTML = '<span class="word-spoken">Waiting for cue...</span>';
            return;
        }
        ...
        // We will output the script differently if there are dialogue lines vs monolithic text
        let scriptHtml = '';

        if (activeCue.dialogue_lines && activeCue.dialogue_lines.length > 0) {
            // Dialogue rendering logic
            activeCue.dialogue_lines.forEach(obj => {
                scriptHtml += `<span class="speaker-name">${obj.character}</span> <span class="dialogue-text">${obj.line}</span> `;
            });
        } else {
            // Fallback to text
            scriptHtml = `<span class="dialogue-text">${activeCue.script_full || activeCue.script_line || ''}</span>`;
        }

        // Split text into words to pace them correctly
        ...
        childNodes.forEach(node => {
            if (node.nodeType === Node.TEXT_NODE) {
                const textWords = node.textContent.split(/\s+/).filter(w => w.length > 0);
                textWords.forEach(w => wordsOutput.push({ text: w, type: 'text' }));
            } else if (node.nodeType === Node.ELEMENT_NODE) {
                // If it's a speaker span, keep it as a full block
                const cls = node.className;
                wordsOutput.push({ text: node.outerHTML, type: 'html', isSpeaker: cls.includes('speaker-name') });
            }
        });

        const numTextWords = wordsOutput.filter(w => w.type === 'text' || w.isSpeaker).length;
        const currentWordIdx = Math.floor(progress * numTextWords);

        const cueKey = `${activeCue.id}-${numTextWords}`;
        if (tpText.dataset.cueKey !== cueKey) {
            ...
            wordsOutput.forEach((item, i) => {
                if (item.type === 'html') {
                    if (item.isSpeaker) {
                        finalHtml += item.text + ' ';
                        wordCounter++; // treat speaker tag as one word timing tick
                    } else {
                        finalHtml += item.text + ' ';
                    }
                } else {
                    finalHtml += `<span class="word" data-idx="${wordCounter}">${item.text} </span>`;
                    wordCounter++;
                }
            });
            tpText.innerHTML = finalHtml;
        }
        ...
        wordEls.forEach(el => {
            ...
            if (idx <= currentWordIdx) {
                ...
                el.classList.remove('dialogue-text');
            } else {
                el.classList.add('dialogue-text');
                el.classList.remove('word-spoken');
            }
        });

        // Autoscroll
        const latestSpoken = Array.from(wordEls).findLast(el => el.classList.contains('word-spoken'));
        if (latestSpoken) {
            const containerRect = tpText.getBoundingClientRect();
            const wordRect = latestSpoken.getBoundingClientRect();
            ...
            if (Math.abs(tpText.scrollTop - scrollTopTarget) > 1) {
                tpText.scrollTop += (scrollTopTarget - tpText.scrollTop) * 0.1;
            }
        }
    }

    updateFixtureStatusUI() {
        const list = document.getElementById('fixture-groups');
        if (!list) return;
        ...
        Object.keys(groups).sort().forEach(groupName => {
            html += `<div class="fixture-group">
                        <div class="fixture-group-title">${groupName.replace('_', ' ')}</div>`;
            groups[groupName].forEach(mesh => {
                ...
                html += `<div class="fixture-row">
                            <span class="fixture-name">${mesh.userData.fixtureId}</span>
                            <div class="fixture-indicator">
                                <div class="fixture-dot" style="background:${colorHex}"></div>
                                <div class="fixture-value">${intensity}%</div>
                            </div>
                         </div>`;
            });
            html += `</div>`;
