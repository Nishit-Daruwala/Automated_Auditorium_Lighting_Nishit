...
if not os.path.exists(INSTRUCTIONS_PATH):
    print(f"âš ï¸ Warning: {INSTRUCTIONS_PATH} not found. Waiting for backend to launch...")
...
# --- INSTRUCTION GROUP â†’ FIXTURE TYPE MAPPING ---
...
class CueEngine:
    ...
    def load_instructions(self):
        ...
        try:
            ...
            for i, scene in enumerate(instructions):
                ...
                emotion = scene.get("emotion", "â€”") # Main field, else meta
                
                # 3. Get Timing â€” keys are 'start_time' and 'end_time' (not 'start'/'end')
                ...
                time_str = f"{start_time:.0f}s â€“ {end_time:.0f}s"
                ...
                display_text = (
                    f"{scene_id} â”‚ {emotion.upper()} â”‚ \"{script_text}\""
                )
            ...
            print(f"âœ… Loaded {len(self.cues)} lighting cues.")
            if len(self.cues) > 0:
                print(f"   Example Cue 1 Text: {self.cues[0]['text']}")

        except Exception as e:
            print(f"âŒ Error loading instructions: {e}")
            import traceback
            traceback.print_exc()
    ...
    def override_theme(self, idx, new_theme):
        ...
        if 0 <= idx < len(self.cues):
            ...
            # Update display text
            parts = cue['text'].split('â”‚')
            if len(parts) >= 3:
                cue['text'] = f"{parts[0].strip()} â”‚ {new_theme.upper()} â”‚ {parts[2].strip()}"
    ...
    def insert_start_and_end(self, end_mode):
        ...
        start_cue = {
            "id": -1,
            "text": "START â”‚ BLACKOUT â”‚ \"Preparing Simulation...\"",
        ...
        if end_mode == "neutral":
            end_cue = {
                "id": -1,
                "text": "END â”‚ NEUTRAL â”‚ \"Simulation Complete.\"",
            ...
            end_cue = {
                "id": -1,
                "text": "END â”‚ BLACKOUT â”‚ \"Simulation Complete.\"",
    ...
    def _sync_clock_to_cue(self, target_idx):
        if self.sim_start_time is None or target_idx >= len(self.cues):
            return
        target_cue = self.cues[target_idx]
        target_start = target_cue.get("start_time", 0)
        desired_sim_elapsed = target_start
        current_time = time.time()
        if self.is_holding and self.hold_pause_time is not None:
            current_time = self.hold_pause_time
        self.sim_start_time = current_time - self.total_paused_duration - desired_sim_elapsed

    def next_cue(self):
        if self.current_index < len(self.cues) - 1:
            self.current_index += 1
            self._sync_clock_to_cue(self.current_index)
        ...
        async for message in websocket:
            ...
            if cmd == "NEXT":
                ...
                if idx is not None and 0 <= idx < len(engine.cues):
                    ...
                    engine._sync_clock_to_cue(idx)
                    changed = True
            elif cmd == "HOLD":
                if not engine.is_holding:
                    # Entering hold â€” record pause time
                    engine.hold_pause_time = time.time()
                else:
                    # Resuming â€” accumulate paused duration
                ...
                status = "â¸ HOLDING" if engine.is_holding else "â–¶ RESUMED"
                print(f"  {status}")
                changed = True
            elif cmd == "RELOAD":
                ...
                engine.hold_pause_time = None
                print(f"ðŸ• Simulation started at {time.strftime('%H:%M:%S')}")
                changed = True
    ...
    while True:
        if not engine.is_holding and engine.sim_start_time is not None and engine.current_index < len(engine.cues) - 1:
            ...
            if sim_elapsed >= next_start:
                ...
                # Not yet time for next cue â€” send periodic updates
...
async def main():
    async with websockets.serve(handler, HOST, PORT):
        print(f"ðŸŽ­ Lighting Console running on ws://{HOST}:{PORT}")
